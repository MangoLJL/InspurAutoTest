...
class NewCheck(object):
    ...
    def second_step(self):
        ...
        if data_exsists:
            ...
            enterprise_name = self.driver.find_element_by_xpath('//*[@id="grid"]/tbody/tr[1]/td[3]/span').text
    ...
    def fourth_step_check_template(self, template_ID):
        ...
        self.driver.switch_to.default_content()
        self.driver.switch_to.frame("mainFrame")
        self.driver.find_element_by_xpath('//*[@class="table tableBordered gridClause dataTable no-footer"]/tbody/tr/td[11]/button').click()
        time.sleep(1)
        self.common_action.scroll_and_switch_to_iframe()
...
class Template(object):
    ...
    def create_template(self):
        ...
        drug_template_name = ("%ssunhr测试模板" % time.strftime('%Y%m%d%H%M%S', time.localtime(time.time())))
        self.driver.find_element_by_id('templateName').send_keys(drug_template_name)
        self.button.click('businessOffices0')
        ...
        self.send_keys.send('//*[@id="grid"]/tbody/tr/td[11]/input', 'sunhr测试评分标准')
        self.send_keys.send('//*[@id="grid"]/tbody/tr/td[12]/input', '100')
        iframe = self.driver.find_element_by_xpath('//iframe[1]')
        self.driver.switch_to.frame(iframe)
        self.driver.find_element_by_xpath("//button[@class='btn btn-success'][2]").click()
        self.button.click_confirm_button()
        globalvar.set_value('drug_template_name', drug_template_name)
    ...
    def clean_template(self):
        ...
        time.sleep(0.5)
        self.button.click_confirm_button()
        print(time.strftime('%Y-%m-%d_%H-%M-%S', time.localtime(time.time())) + '清理药品模板完成')
