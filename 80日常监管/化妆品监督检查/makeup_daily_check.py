# coding=utf-8
import re
import time
from functools import reduce
from common_action import Setup, SwitchToFrame, CommonAction
from makeup_actions import NewCheck, NewDoubleRandom, NewNormalTask
from bs4 import BeautifulSoup
from selenium import webdriver
from selenium.webdriver.chrome.options import Options
from selenium.webdriver.common.by import By
from selenium.webdriver.support.ui import WebDriverWait
from selenium.webdriver.common.action_chains import ActionChains
#from selenium.webdriver.common.keys import Keys
from selenium.webdriver.support import expected_conditions as EC
from common_action import Setup, SwitchToFrame, Button, CommonAction


def new_check():
    def true_plus_false(a, b):
        return (a and b)
    try:
        final_true_or_false = [True, True, True]
        ture_or_false = [True, True, True, True, True]
        enterprise_type = ['化妆品生产者', '化妆品经营者', '化妆品互联网经营']
        check_type_name = ['日常监督检查', '随机检查', '跟踪检查', '有因检查', '专项检查']
        for y in range(0, 3):
            if y != 1:  # 食品经营特殊化
                for i in range(0, 5):
                    try:
                        print(time.strftime('%Y-%m-%d_%H-%M-%S', time.localtime(time.time())) + '开始进行%s%s测试' % (enterprise_type[y], check_type_name[i]))
                        makeup_new_check_setup = Setup('http://10.12.1.80/portal/jsp/public/login.jsp')
                        driver = makeup_new_check_setup.setup_driver('liubx', '1', '智慧监管', '日常监管')
                        makeup_new_check_setup.choose_menu('化妆品监督检查', '新建检查', '现场录入')
                        switch_to_frame = SwitchToFrame(driver)
                        switch_to_frame.switch_to_main_frame()
                        new_check = NewCheck(driver)
                        new_check.first_step(y)
                        data_exsists = new_check.second_step()
                        if data_exsists:
                            checkTypeCode = 'checkTypeCode' + str(i)
                            new_check.third_step(checkTypeCode)
                            check_situation = new_check.fourth_step()
                            new_check.fifth_step()
                            new_check.final_step()
                            new_check_confirmer = NewCheck(driver)
                            ture_or_false[i] = new_check_confirmer.confirm_new_check(check_situation, checkTypeCode)  # 共五个
                            driver.quit()
                        else:
                            print(time.strftime('%Y-%m-%d_%H-%M-%S', time.localtime(time.time())) + '%s企业列表无数据，跳过此类别企业...' % enterprise_type[y])
                            driver.quit()
                            break
                    except Exception as e:
                        ture_or_false[i] = False
                        print(time.strftime('%Y-%m-%d_%H-%M-%S', time.localtime(time.time())) + '测试%s%s出错，截图已保存,错误信息为%s' % (enterprise_type[y], check_type_name[i], e))
                        driver.get_screenshot_as_file("C:\\Users\\Administrator\\Documents\\PythonAutoTest\\80日常监管\\食品日常检查\\error_screenchot\\%s%s%snew_check.png" % (
                            time.strftime('%Y-%m-%d_%H-%M-%S', time.localtime(time.time())), enterprise_type[y], check_type_name[i]))
                        continue
                ture_or_false_suite = reduce(true_plus_false, ture_or_false)
                final_true_or_false[y] = ture_or_false_suite
            else:
                makeup_bussiness_type = ['商场', '超市', '专卖店', '批发市场', '母婴用品店', '小商店', '网络销售',
                                         '美容院', '美发店', '美体店', '宾馆', '酒店', '游泳馆', '洗浴中心', '其他', ]
                print(time.strftime('%Y-%m-%d_%H-%M-%S', time.localtime(time.time())) + '开始进行化妆品经营者十五个小类的测试')
                makeup_bussiness_suite_true_or_false = [[], [], []]
                for z in range(0, 15):
                    print(time.strftime('%Y-%m-%d_%H-%M-%S', time.localtime(time.time())) + '开始进行%s测试' % makeup_bussiness_type[z])
                    makeup_new_check_setup = Setup('http://10.12.1.80/portal/jsp/public/login.jsp')
                    driver = makeup_new_check_setup.setup_driver('liubx', '1', '智慧监管', '日常监管')
                    common_action = CommonAction(driver)
                    makeup_new_check_setup.choose_menu('化妆品监督检查', '新建检查', '现场录入')
                    switch_to_frame = SwitchToFrame(driver)
                    switch_to_frame.switch_to_main_frame()
                    new_check = NewCheck(driver)
                    new_check.first_step(y)

                    enterprise_selector = driver.find_element_by_id("enterpriseName")
                    ActionChains(driver).double_click(enterprise_selector).perform()
                    driver.switch_to.default_content()
                    time.sleep(1)
                    iframe = driver.find_element_by_xpath("//iframe[contains(@id,'layui-layer-iframe')]")
                    driver.switch_to.frame(iframe)
                    time.sleep(3)
                    data_exsists = common_action.data_exsists()
                    if data_exsists:
                        enterprise_radio_button = WebDriverWait(driver, 10, 0.5).until(EC.presence_of_element_located((By.XPATH, "//html//tr[1]/td[2]/input[1]")))
                        enterprise_radio_button.click()
                        driver.find_element_by_xpath("//button[@class='btn btn-success']").click()
                        time.sleep(1)
                        driver.switch_to.default_content()
                        driver.switch_to.frame("mainFrame")
                        businessItem = "businessItem" + str(z)
                        driver.find_element_by_id(businessItem).click()
                        driver.find_element_by_id("secondBtn").click()
                        driver.find_element_by_id()
                    else:
                        print(time.strftime('%Y-%m-%d_%H-%M-%S', time.localtime(time.time())) + '%s企业列表无数据，跳过%s类别企业...' % (enterprise_type[y], enterprise_type[y]))
                        driver.quit()
                        break
                    checkTypeCode = 'checkTypeCode' + str(z)
                    new_check.third_step(checkTypeCode)
                    check_situation = new_check.fourth_step()
                    new_check.fifth_step()
                    new_check.final_step()
                    new_check_confirmer = NewCheck(driver)
                    makeup_bussiness_suite_true_or_false[z].append(new_check_confirmer.confirm_new_check(check_situation, checkTypeCode))  # 每组五个
                    driver.quit()
                makeup_bussiness_middle_ture_or_false = []
                for z in range(0, 3):
                    makeup_bussiness_middle_ture_or_false.append(reduce(true_plus_false, makeup_bussiness_suite_true_or_false[z]))  # 每组三个
                makeup_bussiness_final_ture_or_false = reduce(true_plus_false, makeup_bussiness_middle_ture_or_false)  # 食品经营最终结果，一个
                final_true_or_false[y] = makeup_bussiness_final_ture_or_false
        return (reduce(ture_plus_false, final_true_or_false))
    except Exception as e:
        print("%s%s测试未通过，截图已保存至new_check.png，错误信息：" % (enterprise_type[y], check_type_name[i]), e)
        driver.get_screenshot_as_file("C:\\Users\\Administrator\\Documents\\PythonAutoTest\\80日常监管\\食品日常检查\\error_screenchot\\%snew_check.png" %
                                      time.strftime('%Y-%m-%d_%H-%M-%S', time.localtime(time.time())))


def double_random_task():
    try:
        makeup_new_random_task_setup = Setup('http://10.12.1.80/portal/jsp/public/login.jsp')
        driver = makeup_new_random_task_setup.setup_driver('liubx', '1', '智慧监管', '日常监管')
        makeup_new_random_task_setup.choose_menu('食品监督检查', '任务管理', '双随机任务')
        switch_to_frame = SwitchToFrame(driver)
        new_double_random = NewDoubleRandom(driver)
        switch_to_frame.switch_to_main_frame()
        task_name = new_double_random.create_new_random_task()  # 创建双随机任务
        new_random_test_confirmer_setup = Setup('http://10.12.1.80/portal/jsp/public/login.jsp')
        driver = new_random_test_confirmer_setup.setup_driver('liubx', '1', '智慧监管', '日常监管')  # 因为涉及到计划提交到其他人的情况，所以需要另外建一个driver
        new_random_test_receiver = NewDoubleRandom(driver)
        new_random_test_receiver.receive_new_random_test(task_name)  # 接收双随机任务
        driver.quit()
        new_random_test_confirmer_setup = Setup('http://10.12.1.80/portal/jsp/public/login.jsp')
        driver = new_random_test_confirmer_setup.setup_driver('liubx', '1', '智慧监管', '日常监管')
        new_random_test_checker = NewDoubleRandom(driver)
        enterprise_name = new_random_test_checker.check_new_random_test(task_name)
        ture_or_false = new_random_test_checker.confirm_random_enterprise_check(task_name, enterprise_name)
        return ture_or_false
        driver.quit()
    except Exception as e:
        print("测试未通过，截图已保存至double_random_task_error.png，错误信息：", e)
        driver.get_screenshot_as_file("C:\\Users\\Administrator\\Documents\\PythonAutoTest\\80日常监管\\食品日常检查\\error_screenchot\\%sdouble_random_task_error.png" %
                                      time.strftime('%Y-%m-%d_%H-%M-%S', time.localtime(time.time())))


def normal_task():
    try:
        makeup_new_random_task_setup = Setup('http://10.12.1.80/portal/jsp/public/login.jsp')
        driver = makeup_new_random_task_setup.setup_driver('liubx', '1', '智慧监管', '日常监管')
        makeup_new_random_task_setup.choose_menu('食品监督检查', '任务管理', '计划管理')
        switch_to_frame = SwitchToFrame(driver)
        switch_to_frame.switch_to_main_frame()
        new_normal_task = NewNormalTask(driver)
        normal_plan_name = new_normal_task.create_task()
        new_normal_task_confirmer = NewNormalTask(driver)
        ture_or_false = new_normal_task_confirmer.confirm_new_normal_task(normal_plan_name)
        driver.quit()
        return ture_or_false
    except Exception as e:
        print("测试未通过，截图已保存至normal_task_error.png，错误信息：", e)
        driver.get_screenshot_as_file("C:\\Users\\Administrator\\Documents\\PythonAutoTest\\80日常监管\\食品日常检查\\error_screenchot\\%snormal_task_error.png" %
                                      time.strftime('%Y-%m-%d_%H-%M-%S', time.localtime(time.time())))
